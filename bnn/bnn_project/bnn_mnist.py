...
class BNN_MNIST:
    ...
    def create_golden_h(self, num_samples=10):

        mnist = np.load("dataset/mnist_test_data_original.npy", allow_pickle=True)
        X_all = mnist.item().get("data")
        y_all = mnist.item().get("label")
        ...
        for i in range(num_samples):
            ...
            X0_q = self.sign(self.adj(x))

            X0_int = X0_q.astype(int).tolist()
            
            inputs.append(X0_int)
            
            out = self.feed_forward_quantized(np.array([x]))[0].astype(int).tolist()
            
            golden.append(out)

        w1 = self.fc1w_q.astype(int).tolist()
        w2 = self.fc2w_q.astype(int).tolist()
        w3 = self.fc3w_q.astype(int).tolist()
        ...
        with open("../hls/golden.h", "w") as f:
            ...
            f.write("static const int fc1w[128][784] = {\n")
            ...
            f.write("static const int fc2w[64][128] = {\n")
            ...
            f.write("static const int fc3w[10][64] = {\n")
            ...
            f.write(f"static const int inputs[NUM_SAMPLES][784] = {{\n")
            ...
            f.write(f"static const int golden_outputs[NUM_SAMPLES][10] = {{\n")
...
if __name__ == "__main__":
    ...
    if run_option == 1:
        ...
        #bnn.hlscode()
        bnn.create_golden_h(num_samples=10)
    elif run_option == 4:
